...
def hex_to_rgb(color):
	...
	color=color[1:]
	return 'rgb'+str(tuple(c for c in codecs.decode(color, 'hex')))
...
def rgb_to_hex(color):
	...
	return codecs.encode('#'+''.join(map(chr, rgb)), 'hex')
...
def hex_to_hsv(color):
	...
	color=normalize(color)
	color=color[1:]
	color=tuple(c/255.0 for c in codecs.decode(color, 'hex'))
...
def color_range(color,N=20):
	...
	for c in HSV_tuples:
		...
		hex_out.append("#"+"".join([codecs.encode(chr(x), 'hex') for x in c]))
